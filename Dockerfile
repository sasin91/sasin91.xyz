...
COPY --from=build --chown=nextjs:nodejs /var/www/sasin91/.next/standalone ./
# copy the public folder from the project as this is not included in the build process
COPY --from=build --chown=nextjs:nodejs /var/www/sasin91/public ./apps/nextjs/public
# copy the static folder inside the .next folder generated from the build process 
COPY --from=build --chown=nextjs:nodejs /var/www/sasin91/.next/static ./apps/nextjs/.next/static
COPY --from=build --chown=nextjs:nodejs /var/www/sasin91/.next/static ./apps/nextjs/.next/static
# COPY --from=build --chown=nextjs:nodejs /var/www/sasin91 ./full
RUN chown -R nextjs:nodejs /var/www/sasin91
